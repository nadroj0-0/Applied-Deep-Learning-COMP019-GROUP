...
QUANTILES = [0.005, 0.025, 0.165, 0.25, 0.5, 0.75, 0.835, 0.975, 0.995]  # M5 WSPL 9-quantile set
...
def compute_item_weights(data_dir: str, top_k: int, sampling: str = "all",
    ...
    """
    Revenue weights aligned with Yen's pipeline and M5 WSPL spec.
    Uses daily sales * daily price over the last `last_n_days` of training,
    then normalises to sum to 1. Matches BaseModel.load_and_split_data() exactly.
    """
...
def evaluate_model(model_name: str, run_dir: Path,
    ...
    # split_protocol from saved config — eval must use same split as training
    split_protocol  = str(cfg.get("split_protocol",  "yen_v1"))
    weight_protocol = str(cfg.get("weight_protocol", "yen_v1"))
    _, _, test_loader, stats, _, feature_index = build_dataloaders(
        data_dir        = data_dir,
        seq_len         = seq_len,
        horizon         = horizon,
    ...
    # n_features and feature_index must match training — use saved cfg
    train_feature_set    = str(cfg.get("feature_set", "sales_only"))
    cfg["n_features"]    = len(get_feature_cols(train_feature_set))
    cfg["vocab_sizes"]   = cfg.get("vocab_sizes",   {})
    cfg["feature_index"] = feature_index   # from build_dataloaders — hierarchical models need this
        ...
        with torch.no_grad():
            for x, y in test_loader:
                ...
                if is_nb:
                    ...
                    # seq2seq output: (B, H, Q) — horizon × quantiles
                    # extract median for point metrics, keep full tensor for coverage/CRPS
                    q_out = model(x).cpu()                        # (B, H, Q)
                    all_preds.append(q_out[:, :, median_idx])     # (B, H) — median across horizon
