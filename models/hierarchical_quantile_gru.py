...
class HierarchicalQuantileGRU(nn.Module):
    def __init__(self, input_dim=6, hidden_dim=64, num_layers=2, num_quantiles=9):
        super().__init__()
...
class HierarchicalQuantileGRUModel(BaseModel):
    @property
    def model_name(self): return "hierarchical_quantile_gru"

    def preprocess(self):
        print(f"Preprocessing {self.model_name}...")
        cat_mappings = {col: {val: i for i, val in enumerate(self.train_raw[col].unique())} 
                        for col in ['state_id', 'store_id', 'cat_id', 'dept_id']}
        weight_dict = self.item_weights.to_dict()
        self.train_processed = M5TimeSeriesDataset(self.train_raw, cat_mappings, weight_dict)
        self.val_processed   = M5TimeSeriesDataset(self.val_raw, cat_mappings, weight_dict)
        self.test_processed  = M5TimeSeriesDataset(self.test_raw, cat_mappings, weight_dict)
    ...
    def train(self, epochs=5, lr=0.001, batch_size=4096):
        ...
        self.model = HierarchicalQuantileGRU(input_dim=6, num_quantiles=len(self.QUANTILES)).to(self.device)
    ...
    def predict(self):
        ...
        preds_array = np.clip(preds_array, a_min=0, a_max=None)
        preds_array = np.sort(preds_array, axis=1) # Fast monotonicity
