...
class BaseModel(ABC):
    ...
    def _build_pinball_tensor(self, preds_df: pd.DataFrame):
        """
        Shared setup for WSPL and CRPS. 
        Highly optimized to bypass 54-million row Pandas groupbys.
        """
        q_cols  = [f"q{q}" for q in self.QUANTILES]

        # 1. Extract test targets for d_1914-d_1941 (28 days)
        test_targets = (
        ...
        # 2. Reshape predictions to match
        preds_pivot = (
        ...
        # 3. --- FAST SCALE COMPUTATION ---
        # Instead of grouping 54 million rows in Pandas, we reshape into a dense matrix
        num_items = self.train_raw['id'].nunique()
        num_days = len(self.train_raw) // num_items
        
        # Matrix of shape (30490 items, 1773 days)
        sales_matrix = self.train_raw['sales'].values.reshape(num_items, num_days)
        
        # Calculate absolute difference day-to-day for each item (np.diff operates across axis=1)
        abs_diff = np.abs(np.diff(sales_matrix, axis=1))
        
        # Mean absolute difference per item
        scale_array = abs_diff.mean(axis=1)
        
        # Map back to exactly the order of IDs we are evaluating
        unique_train_ids = self.train_raw['id'].iloc[0::num_days].values
        scale_series = pd.Series(scale_array, index=unique_train_ids)
        scale = scale_series.reindex(ids).clip(lower=1e-8)
